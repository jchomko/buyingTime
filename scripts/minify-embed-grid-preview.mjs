...
const inputPath = path.join(root, "public", "embed-grid-preview.html")
const outDir = path.join(root, "public", "generated")
const minHtmlPath = path.join(outDir, "embed-grid-preview.inline.min.html")

const html = await readFile(inputPath, "utf8")
const moduleScriptMatch = html.match(/<script\s+type=["']module["']>([\s\S]*?)<\/script>/i)
if (!moduleScriptMatch) {
  throw new Error("Could not find <script type='module'> block in embed-grid-preview.html")
}
...
const bundleResult = await build({
  stdin: {
    ...
    resolveDir: root,
    sourcefile: "embed-grid-preview.entry.js",
    loader: "js"
  },
  bundle: true,
  write: false,
...
console.log("Embed grid preview bundled + minified (esbuild + terser).")
